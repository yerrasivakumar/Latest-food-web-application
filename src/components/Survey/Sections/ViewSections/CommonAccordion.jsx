  ...
  dispatch(setGenderStatus({
   ...
   const handleSave = () =>{
    ...
    appState?.survey?.survey?.sections[memberIndex]?.questions?.map((quest,index)=>{
      ...
      if((!appState?.survey?.survey?.sections[memberIndex].conditions[0]?.adult_status || !appState?.survey?.survey?.sections[memberIndex].conditions[0]?.gender_status) && showToast){
        toast(`${t('Please Select Age and Gender Status')}`, {
          position: "top-center",
          autoClose: 4000,
          hideProgressBar: false,
          pauseOnHover: true,
